...
`isoTime` 现在同时校验月份、当月实际天数、小时（0–23）、分钟（0–59）和秒（0–59），避免 JavaScript `Date.parse` 对不存在时间进行规范化。保留合法的 UTC、毫秒和数值时区偏移 ISO 字符串。

新增 `2026-02-30`、`24:00:00`、`23:60:00` 和 `23:59:60` 回归测试；修复后 Task 4 数据测试 16/16 通过，全套测试 37/37 通过，`npm run build` 通过。
